...
import { addToCart, getProductsBySearch } from "../../actions";
import flipkertSupercoine from "../../images/logo/resultnotfoundPic.png";
...
function SearchResult(props) {
    ...
    const dispatch = useDispatch();        
        ...
        useEffect(() => {
            const { match } = props
            const searchText ={
                searchInput : match.params.searchItem 
            }
    ...
    return (
        <Layout>
        {
          product.loading ? null : product.products.length > 0 ?
        <LayoutWithFilter>
            <Card
                style={{
            ...
            >
                {product.products && product.products.map((product, index) => (
                    <div className="productDetailsMainCont" key={index} >
        ...
        >
            <img style={{width : "300px"}} src={flipkertSupercoine} />
            <div
              style={{
                  fontSize : "26px",
                  lineHeight : "36px",
            ...
            >Please check the spelling or try searching for something else</div>
        </div>
    }
        </Layout>
    )
